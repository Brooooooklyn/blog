...
function loadAllPosts(): Post[] {
  ...
  for (const [path, raw] of Object.entries(mdFiles)) {
    ...
    const slug = path.split("/").at(-2) ?? data.postname
    posts.push({
      data: {
        title: data.title,
...
export function getAllPosts(): Post[] { return loadAllPosts() }
export function getPostsByLang(lang: Lang): Post[] { return getAllPosts().filter((p) => p.data.lang === lang) }
export function getPostByName(postname: string, lang: Lang): Post | null { return getAllPosts().find((p) => p.data.postname === postname && p.data.lang === lang) ?? null }

export function getReadingTime(content: string): number {
  const cjkChars = (content.match(/[\u4e00-\u9fff\u3400-\u4dbf]/g) || []).length
  const latinWords = content.replace(/[\u4e00-\u9fff\u3400-\u4dbf]/g, "").split(/\s+/).filter((w) => w.length > 0).length
  return Math.max(1, Math.ceil((cjkChars + latinWords) / 300))
}

export function langPrefix(lang: Lang): string { return lang === DEFAULT_LANG ? "" : `/${lang}` }
export function getPostUrl(post: { data: { postname: string } }, lang: Lang): string { return `${langPrefix(lang)}/${post.data.postname}` }

export function getAllTags(lang: Lang): Map<string, number> {
  const posts = getPostsByLang(lang)
  const tags = new Map<string, number>()
  for (const post of posts) { for (const tag of post.data.tags ?? []) { tags.set(tag, (tags.get(tag) ?? 0) + 1) } }
  return tags
}
